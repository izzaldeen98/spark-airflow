...
spark = (
    ...
    .config("spark.hadoop.fs.s3a.endpoint", f"{minio_url}")
    ...
    .config("spark.sql.catalog.nessie.uri", f"{nessie_url}/api/v1")
    .config("spark.sql.catalog.nessie.ref", "main")
    .config(
        "spark.sql.catalog.nessie.warehouse", nessie_data_warehouse_path
    )
    .getOrCreate()
)
